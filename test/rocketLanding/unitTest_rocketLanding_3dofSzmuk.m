...
dbstop if error
%%
discretisationType = 'trapezoidal';
...
BodyMapRocket.DynamicModel.controlVariable = 'thrust';
...
Pobs1   = [170,400,200]';
Pobs2   = [0,300,0]';
glideSlope  = deg2rad(80);
tiltAngleP  = deg2rad(15);
tiltAngle0  = deg2rad(1);
tiltAngleF  = deg2rad(1);
%% User Initial Guess
...
% Scopt.problem.phases(1).dynamics.timeDerivativeFunction  = @(t,X,U,BodyMap,ii) controlMatrixRocketLanding(t,X,U,BodyMap,ii);

% Scopt.problem.phases(1).scale.states   = [R_sc;R_sc;R_sc;V_sc;V_sc;V_sc;1];
% Scopt.problem.phases(1).scale.controls = [0.015;0.015;0.015;0.015];
% Scopt.problem.phases(1).scale.time     = [7*60];

% Bounds

switch BodyMapRocket.DynamicModel.controlVariable
    case 'thrust'
    Scopt.problem.phases(1).bounds.states.upper    = [R0(1)*1.2;R0(2)*1.2;500;70;70;70;MassModel.massWet] ;
    Scopt.problem.phases(1).bounds.states.lower    = [0; -R0(2)*0.2 ;-500;-70;-70;-70;MassModel.massInert] ;
    ...
    case 'acceleration'
    Scopt.problem.phases(1).bounds.states.upper    = [R0(1)*1.2;R0(2)*1.2;500;70;70;70;zU] ;
    Scopt.problem.phases(1).bounds.states.lower    = [0; -R0(2)*0.2 ;-500;-70;-70;-70;zL] ;
...
switch BodyMapRocket.DynamicModel.controlVariable
    case 'thrust'
Scopt.problem.objective.mayer.states(7)    = -1/(0.5*(MassModel.massWet - MassModel.massInert) + MassModel.massInert);
    case 'acceleration'
Scopt.problem.objective.mayer.states(7)    = -1/(0.5*(zU - zL) + zL);
end
        case 2
        Scopt.problem.objective.type               = 'minimize';
        Scopt.problem.objective.scale              = 10*60;
...
Scopt.algorithm.sequential.trustRegion.radius     = 3e1;
Scopt.algorithm.sequential.trustRegion.lambda     = 1e-3;
...
Scopt.algorithm.virtualControl.phases(1).lambda       = 1.0e0;
...
Scopt.algorithm.sequential.trustRegion.phases(1).timeFinal.lambda    = 1e1;
